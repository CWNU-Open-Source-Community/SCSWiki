...
未提供可访问来源时，使用以下待核验元数据；在正文保留资料名称和适用届次：
...
event:
  ...
  schedule: 当届时间待核实
  reviewRequired: true # 缺少可访问来源时明确标记待核验
  checked: '2026-09-12' # 资料核查日期
  order: 10
```

正文包含活动简介、参与入口、时间说明、准备建议和公开来源，并在标题后放置 `<EventDetails />`。当前活动页不提供外部跳转，来源名称保留在正文；缺少可访问来源时须使用 `status: needs-review` 和 `event.reviewRequired: true`。目录和侧边栏自动读取详情页 Front Matter，无需手动添加重复的导航数据。
...
待核验活动统一显示“待核验”，不提示正在报名。其他活动的状态按报名窗口、活动周期和当前时间计算。到达 `end` 的活动自动进入历史归档；报名截止但未开始的活动仍在目录中，标注“报名已截止”。有开始时间但没有结束时间时，不自动推断“进行中”。年度活动若含延长周期，应在正文解释，不能让“进行中”被误解为可报名。页面打开后每分钟刷新时间状态，无需后台服务。
